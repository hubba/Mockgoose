                    ...
                    if (options.upsert) {
                        ...
                        for (var condition in conditions) {
                            if (typeof conditions[condition] === 'object') {
                                for (var obj in conditions[condition]) {
                                    if (conditions[condition].hasOwnProperty(obj) &&
                                        ['$gt', '$lt'].indexOf(obj) !== -1) {
                        ...
                        conditionsToDelete.forEach(function (x) {
                            delete conditions[x];
                        });
                        conditions._id = new ObjectId();
                        //add setOnInsert to the update
                        if (!!update.$setOnInsert) {
                            for(var obj in update.$setOnInsert) {
                                conditions[obj] = update.$setOnInsert[obj];
                            }
                        }
